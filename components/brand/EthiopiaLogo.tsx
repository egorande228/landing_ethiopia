...
type EthiopiaLogoSize = "sm" | "lg";

type EthiopiaLogoProps = {
  ariaLabel?: string;
  animated?: boolean;
  className?: string;
  decorative?: boolean;
  glow?: boolean;
  size?: EthiopiaLogoSize;
};
...
const widthBySize: Record<EthiopiaLogoSize, string> = {
  sm: "124px",
  lg: "148px",
};

const frameWidthBySize: Record<EthiopiaLogoSize, string> = {
  sm: "144px",
  lg: "168px",
};

const paddingXBySize: Record<EthiopiaLogoSize, string> = {
  sm: "10px",
  lg: "12px",
};

const paddingYBySize: Record<EthiopiaLogoSize, string> = {
  sm: "8px",
  lg: "9px",
};
  ...
  return (
    <span
      ...
      style={
        {
          display: "inline-flex",
          width: frameWidthBySize[size],
          maxWidth: "100%",
          padding: `${paddingYBySize[size]} ${paddingXBySize[size]}`,
          "--brand-logo-width": widthBySize[size],
          "--brand-logo-frame-width": frameWidthBySize[size],
          "--brand-logo-pad-x": paddingXBySize[size],
          "--brand-logo-pad-y": paddingYBySize[size],
        } as CSSProperties
      }
    >
      <svg
        ...
        style={{ display: "block", height: "auto", maxWidth: "100%", width: widthBySize[size] }}
        viewBox="0 0 533 94"
      ...
      >
        {letters.map((letter, index) => (
          <g
            className={[
              "brand-logo__letter",
              `brand-logo__letter--${letter.classSuffix}`,
              `brand-logo__letter--${letter.tone}`,
            ].join(" ")}
            data-letter={letter.dataLetter}
            key={letter.dataLetter}
            style={
              {
                "--brand-logo-letter-delay": `${index * 110}ms`,
                "--brand-logo-letter-index": index,
              } as CSSProperties
            }
          >
